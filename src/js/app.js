...
class AuditApp {
    ...
    createCSVForEmail() {
        ...
        // Try to get structured insights data first
        const structuredInsights = this.getStructuredInsightsData();
        if (structuredInsights) {
            // Use structured data for better email formatting
            csvLines.push(`"Résumé Exécutif: ${this.stripMarkdownForCSV(structuredInsights.resume_executif)}"`);
            csvLines.push('');
            csvLines.push('"Points Forts:"');
            structuredInsights.points_forts.forEach(point => {
                csvLines.push(`"- ${point}"`);
            });
            csvLines.push('');
            csvLines.push('"Faiblesses:"');
            structuredInsights.faiblesses.forEach(weakness => {
                csvLines.push(`"- ${weakness}"`);
            });
            csvLines.push('');
            csvLines.push('"Recommandations:"');
            structuredInsights.recommandations.forEach(recommendation => {
                csvLines.push(`"- ${recommendation}"`);
            });
            csvLines.push('');
            csvLines.push(`"Analyse Globale: ${this.stripMarkdownForCSV(structuredInsights.analyse_globale_maturite)}"`);
            csvLines.push('');
            csvLines.push(`"Feuille de Route: ${this.stripMarkdownForCSV(structuredInsights.feuille_de_route)}"`);
        } else {
    ...
    async renderInsights() {
        ...
        try {
            // Appeler la fonction de génération de PDF Gemini
            exportPDF();
            await generateGeminiPdf();
        } catch (error) {
            console.error('Erreur lors de la génération des insights:', error);
    ...
    async exportPDF() {
        // Vérifier si nous avons des insights structurés disponibles
        const insightsData = await generateGeminiPdf();

        if (insightsData) {
            // Utiliser la nouvelle fonction d'export PDF avec données structurées
            exportPDFWithStructuredData(insightsData);
            this.showNotification('Export PDF généré avec données structurées', 'success');
        } else {
    ...
    // Nouvelle méthode pour récupérer les données d'insights structurées
    getStructuredInsightsData() {
        // Cette méthode sera appelée après la génération des insights
        // pour récupérer les données JSON structurées
        return window.lastGeneratedInsights || null;
    }
    

    // Helper method to strip markdown for CSV export
    stripMarkdownForCSV(text) {
        if (!text) return '';
        return text
            .replace(/\*\*(.*?)\*\*/g, '$1')  // Gras
            ...
            .replace(/\[(.*?)\]\(.*?\)/g, '$1') // Liens
            .replace(/"/g, '""')              // Échapper les guillemets pour CSV
            .trim();
    }
